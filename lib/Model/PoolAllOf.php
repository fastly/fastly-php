 ...
 */
class PoolAllOf implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $fastlyModelName = 'pool_allOf';
     ...
     * Sets shield
     *
     * @param string|null $shield Selected POP to serve as a shield for the servers. Defaults to `null` meaning no origin shielding if not set. Refer to the [datacenters API endpoint](/reference/api/utils/datacenter/) to get a list of available POPs used for shielding.
     *
     * @return self
     */
    ...
    public function setQuorum($quorum)
    {
        ...
        if (!is_null($quorum) && ($quorum > 100)) {
            throw new \InvalidArgumentException('invalid value for $quorum when calling PoolAllOf., must be smaller than or equal to 100.');
        }
        if (!is_null($quorum) && ($quorum < 0)) {
            throw new \InvalidArgumentException('invalid value for $quorum when calling PoolAllOf., must be bigger than or equal to 0.');
        }

...
use Fastly\Model\ModelInterface;
...
class ObjectSerializer
{
     ...
     * @param string $type   the ToolsType of the data
     * @param string $format the format of the Tools type of the data
     *
                    ...
                    if ($value !== null && !in_array($fastlyType, ['\DateTime', '\SplFileObject', 'array', 'bool', 'boolean', 'byte', 'double', 'float', 'int', 'integer', 'mixed', 'number', 'object', 'string', 'void'], true)) {
                        ...
                        if (is_callable($callable)) {
                            ...
                            if (!in_array($value, $allowedEnumTypes, true)) {
                                ...
                                throw new \InvalidArgumentException("Invalid value for enum '$fastlyType', must be one of: '$imploded'");
                            }
                        }
                    }
                    if ($value !== null) {
                        $values[$data::attributeMap()[$property]] = self::sanitizeForSerialization($value, $fastlyType, $formats[$property]);
                    }
                }
            } else {
                foreach($data as $property => $value) {
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Fastly\Model\\' . $data->{$discriminator};
            ...
            $instance = new $class();
            foreach ($instance::fastlyTypes() as $property => $type) {

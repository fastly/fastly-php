...
class PurgeApi
{
    ...
    public function bulkPurgeTagRequest($options)
    {
        ...
        $fastly_soft_purge = array_key_exists('fastly_soft_purge', $options) ? $options['fastly_soft_purge'] : 1;
    ...
    public function purgeSingleUrlRequest($options)
    {
        ...
        if ($host === null || (is_array($host) && count($host) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $host when calling purgeSingleUrl'
            );
        }

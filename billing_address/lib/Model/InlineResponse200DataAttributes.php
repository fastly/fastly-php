...
class InlineResponse200DataAttributes implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setCountry($country)
    {
        if (!is_null($country) && (mb_strlen($country) > 2)) {
            throw new \InvalidArgumentException('invalid length for $country when calling InlineResponse200DataAttributes., must be smaller than or equal to 2.');
        }
        if (!is_null($country) && (mb_strlen($country) < 2)) {
            throw new \InvalidArgumentException('invalid length for $country when calling InlineResponse200DataAttributes., must be bigger than or equal to 2.');
        }
        if (!is_null($country) && (!preg_match("/^[A-Z]{2}$/", $country))) {
            throw new \InvalidArgumentException("invalid value for $country when calling InlineResponse200DataAttributes., must conform to the pattern /^[A-Z]{2}$/.");
        }
